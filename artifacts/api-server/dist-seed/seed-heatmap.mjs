...
__export(schema_exports, {
  ...
  appBannersTable: () => appBannersTable,
  bidsTable: () => bidsTable,
  cancellationReasonsTable: () => cancellationReasonsTable,
  earningsTable: () => earningsTable,
  notificationTemplatesTable: () => notificationTemplatesTable,
  otpCodesTable: () => otpCodesTable,
  placesTable: () => placesTable,
  pushTicketsTable: () => pushTicketsTable,
  rewardLevelsTable: () => rewardLevelsTable,
  ridesTable: () => ridesTable,
  serviceAreasTable: () => serviceAreasTable,
...
var usersTable = pgTable("users", {
  ...
  appMode: text("app_mode", { enum: ["rider", "driver"] }).notNull().default("rider"),
  ...
  submittedDocs: jsonb("submitted_docs").$type().notNull().default([]),
...
var ridesTable = pgTable("rides", {
  id: uuid("id").defaultRandom().primaryKey(),
  riderId: uuid("rider_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
...
// ../../lib/db/src/schema/bids.ts
var bidsTable = pgTable("bids", {
  id: uuid("id").defaultRandom().primaryKey(),
  rideId: uuid("ride_id").notNull().references(() => ridesTable.id, { onDelete: "cascade" }),
  driverId: uuid("driver_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
...
var earningsTable = pgTable("earnings", {
  id: uuid("id").defaultRandom().primaryKey(),
  driverId: uuid("driver_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
  rideId: uuid("ride_id").notNull().references(() => ridesTable.id, { onDelete: "cascade" }),
  amount: doublePrecision("amount").notNull(),
...
var adminsTable = pgTable("admins", {
  id: uuid("id").defaultRandom().primaryKey(),
...
var settingsTable = pgTable("app_settings", {
  key: text("key").primaryKey(),
  value: jsonb("value").notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull()
});

// ../../lib/db/src/schema/places.ts
var placesTable = pgTable(
  "places",
  {
    id: uuid("id").defaultRandom().primaryKey(),
...
// ../../lib/db/src/schema/vehicleTypes.ts
var vehicleTypesTable = pgTable("vehicle_types", {
  id: uuid("id").defaultRandom().primaryKey(),
...
// ../../lib/db/src/schema/serviceAreas.ts
var serviceAreasTable = pgTable("service_areas", {
  id: uuid("id").defaultRandom().primaryKey(),
  name: text("name").notNull(),
  country: text("country").notNull().default("Morocco"),
  type: text("type", {
    enum: ["service_area", "restricted_area", "pricing_zone"]
  }).notNull().default("service_area"),
...
// ../../lib/db/src/schema/appContent.ts
var appBannersTable = pgTable("app_banners", {
  id: uuid("id").defaultRandom().primaryKey(),
  title: text("title").notNull(),
  imageUrl: text("image_url"),
  placement: text("placement", {
    enum: ["rider_home", "driver_home", "onboarding"]
  }).notNull().default("rider_home"),
  active: boolean("active").notNull().default(true),
  displayOrder: integer("display_order").notNull().default(0),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull()
});
var cancellationReasonsTable = pgTable("cancellation_reasons", {
  id: uuid("id").defaultRandom().primaryKey(),
  text: text("text").notNull(),
  appliesTo: text("applies_to", { enum: ["rider", "driver", "both"] }).notNull().default("both"),
  active: boolean("active").notNull().default(true),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull()
});
var notificationTemplatesTable = pgTable("notification_templates", {
  id: uuid("id").defaultRandom().primaryKey(),
...
var rewardLevelsTable = pgTable("reward_levels", {
  id: uuid("id").defaultRandom().primaryKey(),
  name: text("name").notNull(),
  minimumTrips: integer("minimum_trips").notNull().default(0),
  minimumRating: doublePrecision("minimum_rating").notNull().default(4),
...
// ../../lib/db/src/schema/pushTickets.ts
var pushTicketsTable = pgTable(
  "push_tickets",
  {
    id: uuid("id").defaultRandom().primaryKey(),
...
// src/scripts/seed-heatmap.ts
var FORCE = process.argv.includes("--force");
var HOTSPOTS = [
  { lat: 25.2048, lng: 55.2708, weight: 30 },
  // Downtown Dubai / Burj Khalifa
  { lat: 25.1972, lng: 55.2744, weight: 20 },
  // DIFC / Financial Centre
  { lat: 25.076, lng: 55.1302, weight: 20 },
  // Dubai Marina
  { lat: 25.2285, lng: 55.3273, weight: 15 },
  // Deira / Old Dubai
  { lat: 25.1124, lng: 55.139, weight: 10 },
  // JBR Beach
  { lat: 25.2582, lng: 55.3093, weight: 5 }
  // scatter / airport area
];
...
function randomCoordNear(centre, spreadDeg = 0.02) {
  return [
    gauss(centre.lat, spreadDeg),
    gauss(centre.lng, spreadDeg)
  ];
}
function buildPolyline(pickup, dropoff) {
  const points = [pickup];
  const steps = 5;
  for (let i = 1; i < steps; i++) {
    const t = i / steps;
    const midLat = pickup[0] + (dropoff[0] - pickup[0]) * t + gauss(0, 3e-3);
    const midLng = pickup[1] + (dropoff[1] - pickup[1]) * t + gauss(0, 3e-3);
    points.push([midLat, midLng]);
  }
  points.push(dropoff);
  return encodePolyline(points);
}
function randomDateWithinHours(maxHoursAgo) {
  return new Date(Date.now() - Math.random() * maxHoursAgo * 60 * 60 * 1e3);
}
function randomDateBetweenDays(fromDaysAgo, toDaysAgo) {
  const msPerDay = 24 * 60 * 60 * 1e3;
  const rangeMs = (toDaysAgo - fromDaysAgo) * msPerDay;
  return new Date(Date.now() - fromDaysAgo * msPerDay - Math.random() * rangeMs);
}
var VEHICLE_CLASSES = ["ride", "comfort", "moto"];
var STATUSES = ["completed", "completed", "completed", "cancelled"];
var PICKUP_LABELS = [
  "Dubai Mall",
  "Burj Khalifa",
  "Dubai Marina Walk",
...
async function main() {
  ...
  const [{ count }] = await db.select({ count: sql`count(*)` }).from(ridesTable).where(eq(ridesTable.pickupAddress, "__heatmap_seed__"));
  ...
  const riders = await db.select({ id: usersTable.id }).from(usersTable).limit(50);
  if (riders.length === 0) {
    console.error(
  ...
  for (let i = 0; i < TOTAL; i++) {
    ...
    const distanceKm = Math.round(
      Math.sqrt(
        Math.pow((pickup[0] - dropoff[0]) * 111, 2) + Math.pow((pickup[1] - dropoff[1]) * 111 * Math.cos(pickup[0] * Math.PI / 180), 2)
      ) * 10
    ) / 10;
    ...
    rides.push({
      ...
      ratingScore: status === "completed" && Math.random() > 0.4 ? Math.floor(Math.random() * 2) + 4 : null,
      createdAt,
      updatedAt: createdAt
    });
  }
